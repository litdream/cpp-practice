...
int main(int argc, char* argv[]) {
    ...
    // Check for DISPLAY environment variable, set to dummy if not present
    if (std::getenv("DISPLAY") == nullptr) {
        std::cout << "No DISPLAY environment variable detected. Enabling Headless (Dummy) Video Driver." << std::endl;
    ...
    SDL_Window* window = SDL_CreateWindow(
        ...
        SCREEN_WIDTH * SCALE, 
        SCREEN_HEIGHT * SCALE, 
        SDL_WINDOW_HIDDEN // Use HIDDEN for testing
    );
    ...
    SDL_Renderer* renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_SOFTWARE); // Use SOFTWARE for headless
    if (!renderer) {
    ...
    std::cout << "[Test 2] Updating Video Surface from Memory..." << std::endl;
    video.update(backbuffer);

    // Verify that we didn't crash and the surfaces were loaded
    // Since it's headless, we can't visually verify easily without saving the surface,
    // but we can check if `SDL_BlitSurface` was called by checking the backbuffer pixels
    // or just checking if `Video::update` runs without error.
    // Actually, I can check if a pixel was modified from the initial background (black).
    // The XPM font has white pixels.
    ...
    std::cout << "  Pixel modification detected: " << (pixelModified ? "YES" : "NO") << std::endl;
    ...
    if (pixelModified) {
        ...
        std::cout << "\n[ERROR] Video Rasterization Failed to Draw Pixels!" << std::endl;
